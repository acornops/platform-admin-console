...
export function createMockWorkspaceDefaults(record) {
  ...
  function execute(method, path, body = {}, query = new URLSearchParams()) {
    ...
    if (method === "POST" && path === "/workspace-defaults") {
      ...
      const item = body.kind === "mcp_server"
        ? {
            ...
            availableIn: [...body.availableIn], source: structuredClone(body.source),
        ...
        : {
            ...
            description: "Imported platform skill",
            availableIn: [...body.availableIn], source: structuredClone(body.source),
    ...
    if (method === "POST" && path === "/workspace-defaults/resolve-skill") {
      return ok({
        files: [{
          path: "SKILL.md",
        ...
        source: {
          provider: body.repoUrl.includes("gitlab") ? "gitlab" : "github",
    ...
    if (match && method === "PATCH") {
      ...
      item.availableIn = [...body.availableIn];
      item.updatedAt = new Date().toISOString();
      record("admin.system.workspace_default.availability.update", null, "workspace_default", item.id, body, {
        kind: item.kind, availableIn: item.availableIn
      });
