...
const allowedAuditMetadata = new Set([
  ...
  "profileKeys", "disabledKeys", "catalogHash"
]);
...
function projectPlatformSetting(value = {}) {
  const key = ["member_discovery", "ai_policy", "user_sign_in_methods", "help_links", "kubernetes_rbac_additions"].includes(value.key) ? value.key : "";
  const projected = {
    key,
  ...
  if (value.overrideValue !== undefined) projected.overrideValue = key === "kubernetes_rbac_additions"
    ? projectKubernetesRbacAdditionsOverride(value.overrideValue)
    : projectPlatformSettingValue(key, value.overrideValue);
...
function projectPlatformSettingValue(key, value = {}) {
  ...
  if (key === "help_links") return projectHelpLinks(value);
  if (key === "kubernetes_rbac_additions") return projectKubernetesRbacAdditions(value);
  return {};
}
...
function projectPlatformSettingConstraints(key, value = {}) {
  ...
  if (key === "help_links") return {
    documentationProtocols: ["https:"],
    supportProtocols: ["https:", "mailto:"],
    maxUrlLength: Math.min(number(value.maxUrlLength) || 2048, 2048)
  };
  if (key === "kubernetes_rbac_additions") return {
    maxAdditions: number(value.maxAdditions),
    maxResourcesPerAddition: number(value.maxResourcesPerAddition),
    allowedVerbs: array(value.allowedVerbs).filter((verb) => ["get", "list", "watch", "create", "patch", "delete"].includes(verb)).map(String),
    wildcardsAllowed: false,
    runtimeEditable: value.runtimeEditable !== false
  };
  return {};
}

function projectHelpLinks(value = {}) {
  return {
    documentationUrl: validHttpsDestination(value.documentationUrl)
      ? String(value.documentationUrl)
      : "https://docs.acornops.dev",
    supportUrl: validSupportDestination(value.supportUrl)
      ? String(value.supportUrl)
      : "https://discord.gg/jBgTy4KhF"
  };
}
...
function validSupportDestination(value) {
...
function projectKubernetesRbacAdditions(value = {}) {
  return {
    additions: array(value.additions).slice(0, 25).map((addition) => ({
      key: string(addition?.key),
      name: string(addition?.name),
      ...(addition?.description ? { description: string(addition.description) } : {}),
      resources: array(addition?.resources).slice(0, 50).map((resource) => ({
        apiGroup: string(resource?.apiGroup),
        ...
        verbs: array(resource?.verbs).filter((verb) => ["get", "list", "watch", "create", "patch", "delete"].includes(verb)).map(String)
      }))
    }))
  };
}

function projectKubernetesRbacAdditionsOverride(value = {}) {
  return {
    upserts: projectKubernetesRbacAdditions({ additions: value.upserts }).additions,
    disabledKeys: array(value.disabledKeys).slice(0, 25).map(String)
  };
}
...
function projectUserSummary(value = {}) {
  return {
    ...
    ...(value.lastLoginAt ? { lastLoginAt: string(value.lastLoginAt) } : {})
  };
}
