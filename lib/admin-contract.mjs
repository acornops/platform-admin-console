...
const allowedAuditMetadata = new Set([
  ...
  "settingKey", "previousVersion", "version", "correlationId"
]);
...
function projectPlatformSetting(value = {}) {
  const key = ["member_discovery", "ai_policy", "password_signup"].includes(value.key) ? value.key : "";
  const projected = {
    key,
...
function projectPlatformSettingConstraints(key, value = {}) {
  ...
  if (key === "password_signup") {
    return {
      allowedValues: array(value.allowedValues).map(Boolean),
      enablementBlockers: array(value.enablementBlockers).map(String)
    };
  }
  if (key === "ai_policy") {
    return {
