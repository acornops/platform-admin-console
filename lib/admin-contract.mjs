...
export function projectAdminResponse(definition, payload) {
  ...
  if (template === "/admin/v1/system/workspace-defaults/resolve-skill") {
    return projectResolvedGitSkill(payload);
  }
...
function projectResolvedGitSkill(value = {}) {
  const source = value.source || {};
  const files = array(value.files);
  if (!validGitSkillFiles(files)
    || !["github", "gitlab"].includes(source.provider)
    || !validGitImportUrl(source.repoUrl)
    || !validGitRef(source.ref)
    || (source.subpath !== undefined && !validGitSubpath(source.subpath))
    || !/^[0-9a-f]{40}$/i.test(source.commitSha || "")) {
    throw new Error("Invalid resolved Git skill response");
  }
  return {
    ...
    source: {
      ...
      ref: source.ref,
      ...(source.subpath ? { subpath: string(source.subpath) } : {}),
      commitSha: string(source.commitSha)
    }
  };
}

function validGitSkillPath(value) {
  if (typeof value !== "string" || !value || value.length > 512 || value.startsWith("/") || value.endsWith("/")) return false;
  const segments = value.replaceAll("\\", "/").split("/");
  return (value === "SKILL.md" || value.endsWith(".md"))
    && segments.every((segment) => segment && segment !== "." && segment !== "..");
}

export function validGitSkillFiles(files) {
  if (!Array.isArray(files) || files.length < 1 || files.length > 16) return false;
  const paths = files.map((file) => file?.path);
  const totalBytes = files.reduce((size, file) =>
    size + (typeof file?.content === "string" ? new TextEncoder().encode(file.content).byteLength : 0), 0);
  return files.every((file) => validGitSkillPath(file?.path)
      && typeof file?.content === "string"
      && new TextEncoder().encode(file.content).byteLength <= 32768)
    && new Set(paths).size === paths.length
    && paths.includes("SKILL.md")
    && totalBytes <= 131072;
}

export function validGitImportUrl(value) {
...
export function validGitSubpath(value) {
...
function projectAuditPage(value = {}) {
  const items = array(value.items)
    ...
    .map((event) => ({
      ...pick(event, ["id", "adminTokenId", "adminActorIssuer", "adminActorSubject", "adminActorEmail", "adminActorDisplayName", "adminActorRole", "authenticationTime", "action", "outcome", "workspaceId", "workspaceName", "subjectType", "subjectId", "reason", "requestId", "occurredAt"]),
