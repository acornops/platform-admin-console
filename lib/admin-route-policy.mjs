...
export const ADMIN_ROUTE_DEFINITIONS = Object.freeze([
  ...
  route("PATCH", "/settings/(member_discovery|ai_policy|user_sign_in_methods)", "/admin/v1/system/settings/:settingKey", "admin:system:write", "write", ["settingKey"]),
  route("DELETE", "/settings/(member_discovery|ai_policy|user_sign_in_methods)", "/admin/v1/system/settings/:settingKey", "admin:system:write", "write", ["settingKey"]),
  ...
  route("POST", "/workspace-defaults/resolve-skill", "/admin/v1/system/workspace-defaults/resolve-skill", "admin:system:write", "write"),
...
export function sanitizedAdminBody(definition, body) {
  if (definition.upstreamTemplate === "/admin/v1/system/workspace-defaults/resolve-skill") {
    if (!plainObject(body)
      || Object.keys(body).some((field) => !["repoUrl"].includes(field))
      || !validGitImportUrl(body.repoUrl)) {
      return { ok: false, message: "A valid HTTPS Git repository or skill URL is required" };
    }
    return { ok: true, body: { repoUrl: body.repoUrl.trim() } };
  }
  ...
  if (definition.upstreamTemplate === "/admin/v1/system/workspace-defaults/:id") {
    const allowedFields = new Set(definition.method === "PATCH" ? ["availableIn", "reason"] : ["reason"]);
    ...
    if (definition.method === "PATCH" && !validAvailability(body.availableIn)) {
      return { ok: false, message: "availableIn is invalid" };
    }
    return { ok: true, body: {
      ...(definition.method === "PATCH" ? { availableIn: canonicalAvailability(body.availableIn) } : {}),
...
function sanitizeWorkspaceDefaultCreate(body) {
  ...
  const sourceAllowed = new Set(["type", "provider", "repoUrl", "ref", "subpath", "commitSha"]);
  if (Object.keys(body).some((field) => !allowed.has(field))
    || !plainObject(body.source) || body.source.type !== "git"
    || !["github", "gitlab"].includes(body.source.provider)
    || Object.keys(body.source).some((field) => !sourceAllowed.has(field))
    || typeof body.source.repoUrl !== "string" || typeof body.source.ref !== "string"
    || !validGitImportUrl(body.source.repoUrl)
    || !validGitRef(body.source.ref)
    || (body.source.subpath !== undefined && !validGitSubpath(body.source.subpath))
    || !/^[0-9a-f]{40}$/i.test(body.source.commitSha || "")
    || !validGitSkillFiles(body.files)
    || body.files.some((file) => !plainObject(file)
      || Object.keys(file).some((field) => !["path", "content"].includes(field)))) {
  ...
  return { ok: true, body: {
    ...
    source: {
      ...
      repoUrl: body.source.repoUrl.trim(),
      ref: body.source.ref,
      ...(body.source.subpath ? { subpath: body.source.subpath } : {}),
      commitSha: body.source.commitSha
    },
