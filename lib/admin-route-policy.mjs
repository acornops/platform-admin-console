...
export const ADMIN_ROUTE_DEFINITIONS = Object.freeze([
  ...
  route("PATCH", "/settings/(member_discovery|ai_policy|user_sign_in_methods|help_links|kubernetes_rbac_additions)", "/admin/v1/system/settings/:settingKey", "admin:system:write", "write", ["settingKey"]),
  route("DELETE", "/settings/(member_discovery|ai_policy|user_sign_in_methods|help_links|kubernetes_rbac_additions)", "/admin/v1/system/settings/:settingKey", "admin:system:write", "write", ["settingKey"]),
...
export function sanitizedAdminBody(definition, body) {
  ...
  if (definition.upstreamTemplate === "/admin/v1/system/settings/:settingKey") {
    ...
    if (!resetting && definition.values?.settingKey === "help_links" && !validHelpLinks(body.value)) {
      return { ok: false, message: "help_links must contain a valid HTTPS documentation URL and HTTPS or mailto support URL" };
    }
...
function validHelpLinks(value) {
  return plainObject(value)
    && Object.keys(value).length === 2
    && validHttpsDestination(value.documentationUrl)
    && validSupportDestination(value.supportUrl);
}
