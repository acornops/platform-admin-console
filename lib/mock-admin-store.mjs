...
import { mockHelpLinksSetting } from "./mock-help-links.mjs";
import { createMockWorkspaceDefaults } from "./mock-workspace-defaults.mjs";
import {
  mockKubernetesRbacDeploymentDefault,
  mockPlatformSetting,
  resolveMockKubernetesRbacAdditions
} from "./mock-kubernetes-rbac.mjs";
...
export function createMockAdminStore() {
  ...
  const users = [
    user("usr_maya", "maya@atlas.example", "Maya Chen", "2025-08-11T08:40:00Z", true, ["oidc"], 2, "2026-07-31T02:15:00Z"),
    user("usr_jules", "jules@cedar.example", "Jules Okafor", "2025-11-03T10:18:00Z", true, ["password", "oidc"], 2, "2026-08-01T09:42:00Z"),
    user("usr_noor", "noor@northstar.example", "Noor Rahman", "2026-03-08T06:25:00Z", true, ["oidc"], 1, "2026-07-28T06:29:00Z"),
    user("usr_arin", "arin@orbit.example", "Arin Silva", "2025-06-21T09:51:00Z", true, ["password"], 1, "2026-07-30T14:08:00Z"),
    user("usr_sam", "sam@lumen.example", "Sam Rivera", "2026-02-03T13:04:00Z", false, ["password"], 1),
    user("usr_ivy", "ivy@atlas.example", "Ivy Tan", "2025-09-17T11:32:00Z", true, ["oidc"], 2, "2026-08-02T01:20:00Z")
  ];
  ...
  const auditEvents = [
    audit("admin.workspace.member.add", "ws_atlas", "user", "usr_noor", "Platform admin granted Owner access to Atlas Research", "2026-07-16T01:44:00Z", { requestedRole: "owner" }),
  ...
  const platformSettings = new Map([
    ["member_discovery", mockPlatformSetting("member_discovery", { mode: "exact_email" }, {
      allowedModes: ["disabled", "exact_email", "directory"]
    })],
    ["ai_policy", mockPlatformSetting("ai_policy", aiDefault, {
    ...
    ["user_sign_in_methods", mockPlatformSetting("user_sign_in_methods", { methods: ["password", "oidc"] }, {
      allowedMethods: ["password", "oidc"],
      methodBlockers: { password: [], oidc: [] }
    })],
    ["help_links", structuredClone(mockHelpLinksSetting)],
    ["kubernetes_rbac_additions", mockPlatformSetting("kubernetes_rbac_additions", mockKubernetesRbacDeploymentDefault, {
      maxAdditions: 25, maxResourcesPerAddition: 50,
      allowedVerbs: ["get", "list", "watch", "create", "patch", "delete"], wildcardsAllowed: false, runtimeEditable: true
    })]
  ]);
  ...
  const workspaceDefaults = createMockWorkspaceDefaults(record);
  ...
  function execute(method, path, body = {}, query = new URLSearchParams()) {
    ...
    const workspaceDefaultResult = workspaceDefaults.execute(method, path, body, query);
    if (workspaceDefaultResult) return workspaceDefaultResult;
    ...
    match = /^\/settings\/(member_discovery|ai_policy|user_sign_in_methods|help_links|kubernetes_rbac_additions)$/.exec(path);
    if (match && (method === "PATCH" || method === "DELETE")) {
      ...
      if (method === "DELETE") {
        ...
        current.value = match[1] === "kubernetes_rbac_additions"
          ? resolveMockKubernetesRbacAdditions(current.deploymentDefault, body.value)
          : structuredClone(body.value);
  ...
  function audit(action, workspaceId, subjectType, subjectId, reason, occurredAt, metadata = {}) {
    ...
    return {
      ...
      ...(subjectId ? { subjectId } : {}),
      ...(subjectType === "user" && subjectId && find(users, subjectId) ? { subjectDisplayName: find(users, subjectId).displayName } : {}),
      ...(reason ? { reason } : {}),
...
function user(id, email, displayName, createdAt, emailVerified, authMethods, workspaceMembershipCount, lastLoginAt) {
  return { id, email, displayName, createdAt, emailVerified, authMethods, workspaceMembershipCount, ...(lastLoginAt ? { lastLoginAt } : {}) };
}
...
function filterAuditEvents(items, query) {
  const mutationActions = (...actions) => new Set(actions.flatMap((action) => [action, `${action}.request`]));
  const actionGroups = {
    platform_settings_modified: [
      { actions: mutationActions("admin.system.setting.update", "admin.system.setting.reset"), subjectIds: new Set(["member_discovery", "user_sign_in_methods", "kubernetes_rbac_additions"]) }
    ],
    llm_provider_defaults_modified: [
      { actions: mutationActions("admin.system.llm_provider_default.update", "admin.system.llm_provider_default.delete") },
      { actions: mutationActions("admin.system.setting.update", "admin.system.setting.reset"), subjectIds: new Set(["ai_policy"]) }
    ],
    workspace_defaults_modified: [
      { actions: mutationActions("admin.system.workspace_default.create", "admin.system.workspace_default.update", "admin.system.workspace_default.delete") }
    ],
    workspace_status_modified: [{ actions: mutationActions("admin.workspace.suspend", "admin.workspace.restore") }],
    workspace_access_modified: [
      { actions: mutationActions("admin.workspace.member.add", "admin.workspace.member.delete", "admin.workspace.member.role.update", "admin.member.role.update") }
    ]
  };
  return items.filter((event) => {
    if (query.get("action") && event.action !== query.get("action")) return false;
    const groupedFilters = actionGroups[query.get("actionGroup")];
    if (query.get("actionGroup") && !groupedFilters?.some((filter) =>
      filter.actions.has(event.action) && (!filter.subjectIds || filter.subjectIds.has(event.subjectId))
    )) return false;
    if (query.get("workspaceId") && event.workspaceId !== query.get("workspaceId")) return false;
    const workspaceQuery = query.get("workspaceQuery")?.trim().toLocaleLowerCase();
    if (workspaceQuery && event.workspaceId !== query.get("workspaceQuery")?.trim() && !event.workspaceName?.toLocaleLowerCase().includes(workspaceQuery)) return false;
