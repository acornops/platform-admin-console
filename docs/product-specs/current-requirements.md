...
| `REQ-WSP-002` | Selecting a workspace keeps the directory visible and opens an accessible, deep-linkable right-side panel. `Workspace Details` shows Kubernetes connections, virtual-machine connections, current plan, creator, and creation date in that order. Kubernetes and VM values render as neutral connected/plan-limit counters derived from the existing plan catalog, reserving red for over-limit exceptions, and `Change Plan` sits directly beside the current plan value. The console exposes no quota policy, quota usage row, quota override, or separate Manage Workspace section. |
