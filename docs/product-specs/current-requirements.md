...
| `REQ-BND-002` | Platform admins receive governance metadata only. Tenant logs, audit records, targets, agents, sessions, runs, prompts, tools, workspace credentials, commands, and workload changes remain unavailable. Credential operations remain limited to the explicit write-only platform-default LLM key capability in `REQ-SET-002`; the MCP-default browser contract accepts and returns no authentication or credential fields. |
...
| ID | Requirement |
| --- | --- |
| `REQ-UX-001` | Reuse the management console's shell, typography, control structure, and interaction vocabulary. Add and import actions use the same secondary icon-button treatment; bounded forms use the same divided dialog header, close control, scrollable body, and Cancel/primary-action footer. Retain the shield-labelled Platform Admin identity, the concise `Admin · AcornOps` browser title, and a noticeable but restrained cream-clay sidebar tint. Overview remains the standalone first destination. `Resource Management` groups Workspaces and Users. `Platform Settings` groups Workspace, AI Providers, and Capabilities. `Governance` contains Admin Audit. Workspace and AI settings use direct `/settings/workspace` and `/settings/ai` routes; legacy `/settings` continues to open Workspace Settings. The bottom administrator identity is a management-aligned account-menu trigger showing the authenticated human's display name or email and current platform role. Its popover repeats the identity, offers System, Light, and Dark appearance preferences, and provides the explicit logout action. Account settings remain absent because platform-admin identity and credentials are IdP-managed and the console has no local account mutation contract. The former sidebar tenant-boundary card is removed; Overview's `Governance data only` banner carries the lock icon and confidentiality reminder. |
...
| `REQ-DEF-001` | `Capabilities` is a dedicated Platform Settings page with exactly two tabs: MCP servers and Skills. It lets a full `platform-admin` add an HTTPS MCP definition or import a pinned, validated Markdown snapshot by pasting one full URL from a deployment-configured GitHub or GitLab host, select one or more `Available in` destinations (`Agents`, `Kubernetes`, and `Virtual machines`), search by source, filter by destination membership, edit availability, and remove a default through fixed `/admin/v1/system/workspace-defaults` routes. Git provider, ref, and subpath are inferred server-side; credentials and browser-supplied API bases are not accepted. Selecting all three destinations is displayed as `All`; `all` is not stored as a destination. The current list is copied once when a new workspace is created; later additions, availability edits, and removals do not change existing workspaces. MCP creation collects only the server name, HTTPS endpoint, and destinations; authentication and credentials remain in each workspace's existing MCP setup. Copied entries appear disabled in existing Management Console inventories until enabled, at which point they become normal workspace-managed items. Viewers are read-only, auditors have no route, responses omit MCP authentication metadata and persisted skill contents, and built-in tools, catalogs, readiness, revisions, publication, raw skill authoring, and per-workspace targeting remain excluded. |
...
The accepted consumer subset is 26 routes and 8 least-privilege scopes, mirrored in `docs/contracts/manifest.json` and enforced by `lib/admin-route-policy.mjs` and `lib/admin-contract.mjs`. Adding an endpoint, query, scope, or payload field is a contract change, not a UI convenience; update producer and consumer artifacts together and record any necessary deviation in `DESIGN.md`.
