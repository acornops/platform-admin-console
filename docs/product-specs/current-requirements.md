...
| `REQ-DEF-001` | `Capabilities` is a dedicated Platform Settings page with exactly two tabs: MCP servers and Skills. It lets a full `platform-admin` add an HTTPS MCP definition or automatically import a pinned, validated Markdown snapshot from a public GitHub or GitLab repository, select one or more `Available in` destinations (`Agents`, `Kubernetes`, and `Virtual machines`), search by source, filter by destination membership, edit availability, and remove a default through fixed `/admin/v1/system/workspace-defaults` routes. Selecting all three destinations is displayed as `All`; `all` is not stored as a destination. The current list is copied once when a new workspace is created; later additions, availability edits, and removals do not change existing workspaces. MCP creation collects only the server name, HTTPS endpoint, and destinations; authentication and credentials remain in each workspace's existing MCP setup. Copied entries appear disabled in existing Management Console inventories until enabled, at which point they become normal workspace-managed items. Viewers are read-only, auditors have no route, responses omit MCP authentication metadata and skill contents, and built-in tools, catalogs, readiness, revisions, publication, private Git hosts, custom Git API bases, raw skill authoring, and per-workspace targeting remain excluded. |
...
The accepted consumer subset is 25 routes and 8 least-privilege scopes, mirrored in `docs/contracts/manifest.json` and enforced by `lib/admin-route-policy.mjs` and `lib/admin-contract.mjs`. Adding an endpoint, query, scope, or payload field is a contract change, not a UI convenience; update producer and consumer artifacts together and record any necessary deviation in `DESIGN.md`.
