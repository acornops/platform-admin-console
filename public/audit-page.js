...
function auditAffectedMarkup(event) { if (event.workspaceId) return `<div class="audit-affected-primary"><span>Workspace</span><strong class="mono">${escapeText(event.workspaceId)}</strong></div>${event.subjectId ? `<div class="audit-context"><span>${escapeText(titleCase(event.subjectType || "Item"))}</span><span class="mono">${escapeText(event.subjectId)}</span></div>` : ""}`; if (event.subjectId) return `<div class="audit-affected-primary"><span>${escapeText(titleCase(event.subjectType || "Item"))}</span><strong class="mono">${escapeText(event.subjectId)}</strong></div>`; return '<div class="audit-affected-primary"><strong>Platform</strong></div>'; }
function auditAffectedText(event) { return [event.workspaceId ? `Workspace ${event.workspaceId}` : "", event.subjectId ? `${titleCase(event.subjectType || "Item")} ${event.subjectId}` : ""].filter(Boolean).join(" · ") || "Platform"; }
